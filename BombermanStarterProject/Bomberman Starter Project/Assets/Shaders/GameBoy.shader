Shader "Custom/GameBoy"
{
	Properties
	{
		...
		_Darkest("Darkest", color) = (0.0588235, 0.21961,
			0.0588235)
		_Dark("Dark", color) = (0.188235, 0.38431, 0.188235)
		_Light("Light", color) = (0.607843, 0.7372549, 0.0588235)
		_Brightest("Brightest", color) = (0.607843, 0.7372549, 0.0588235)
	}
		SubShader
		{
			// No culling or depth
		...
		sampler2D _MainTex;
		float4 _Darkest, _Dark, _Light, _Brightest; 

			float4 frag(v2f i) : SV_Target
			{
			float4 originalColor = tex2D(_MainTex, i.uv); 
			float luma = dot(originalColor.rgb, float3(0.2126,
				0.7152, 0.0722)); 
			...
			float brightest = saturate(lumaTimesThree - 2.0); 
			color = lerp(color, _Brightest, brightest); 
